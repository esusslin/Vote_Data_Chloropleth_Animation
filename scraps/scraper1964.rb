...
if fips.length > 0
	...
	fips.each do |fip|
		...
		driver.get("http://uselectionatlas.org/RESULTS/data.php?fips=" + fip + "&year=1964&datatype=town&def=1&f=0&off=0&elect=0")
		...
		w = headers.index("Unpled.")

		x = headers.index("%Margin")

		y = headers.index("Goldwa.")

		z = headers.index("Johnson")
