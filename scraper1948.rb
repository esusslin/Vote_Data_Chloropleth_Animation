...
if fips.length > 0
	...
	fips.each do |fip|
		...
		driver.get("http://uselectionatlas.org/RESULTS/data.php?fips=" + fip + "&year=1948&datatype=town&def=1&f=0&off=0&elect=0")
		...
		w = headers.index("Thurmo.")

		x = headers.index("%Margin")

		y = headers.index("Dewey")

		z = headers.index("Truman")
