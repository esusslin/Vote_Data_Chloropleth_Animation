...
if fips.length > 0
	...
	fips.each do |fip|
		...
		driver.get("http://uselectionatlas.org/RESULTS/data.php?fips=" + fip + "&year=1916&datatype=town&def=1&f=0&off=0&elect=0")
		...
		ww = headers.index("Benson")

		w = headers.index("Other")

		x = headers.index("%Margin")

		y = headers.index("Hughes")
